...
plot_this.rolling(21).mean().plot(label='optimal')
S_hp.rolling(21).mean().plot(label='human')
plt.legend()
...
'''SINGLE SUBJECT'''
df_S_w2 = df_use_all[df_use_all.user_id==114156].copy()
...
S_all.plot()
plt.scatter(S_all.index,S_all.hum)
plot_this.rolling(7).mean().plot(label='optimal')
S_hp.rolling(7).mean().plot(label='human')
plt.legend()
