...
datasavename='bla_bla'#SO CAN"T OVERWRITE FILES

'''
NOTE ON MANUSCRIPT NUMBERS
THIS FILE WAS CREATED (AS COPY OF ORIGINAL)
TO GENERATE NUMBERS OF Ss and Predicitons for
THE MANUSCRIPT.  YOU WILL FIND THESE IN COMMENTS BELOW BUT SHORT SUMMARY IS HERE:
Prior to processing left-right maxers/miners, we have:
471 predictions for rounds 1-3 and 47 respondents
Processing max/miners doesnt change either number (no min maxers)
Processing for begin wave date we have:
403 predictions
39 participants
Processing for prediction is behind decision date (make prediction for Jan 14 on Jan 16th.)
401 predictions
39 participants

ALSO, 
BELOW GENERATED CSV OF NUMBER OF PREDICTIONS PER SUBJECT and some associated stats.
Mean = 10.33
STD = 15.98
MEDIAN = 3
Skew=2.15
...
'''
ADDED FOR MANUSCRIPT TO SEND TO METACULUS FOR 
INFORMATION ABOUT Ss
'''
pd.Series(df_use_all.user_id.value_counts().index).to_csv('Participant_List_Prior_to_Preprocess.csv',index=False,header=False)

len(df_use_all)
#471 predictions

len(df_use_all.user_id.value_counts())
#47 respondents
...
pd.Series(df_use_all.user_id.value_counts().index).to_csv('Participant_List_Posterior_to_Preprocess.csv',index=False,header=False)
len(df_use_all.user_id.value_counts())
#39 SUBJECTS NOW

'''
LIST OF NUMBER OF PREDICTIONS PER SUBJECT
'''
pd.Series(df_use_all.user_id.value_counts()).to_csv('Participant_Number_Predictions_Posterior_to_Preprocess_.csv',header=True)
'''SOME STATS'''
df_use_all.user_id.value_counts().mean()
df_use_all.user_id.value_counts().std()
plt.hist(df_use_all.user_id.value_counts())
df_use_all.user_id.value_counts().skew()
df_use_all.user_id.value_counts().median()
